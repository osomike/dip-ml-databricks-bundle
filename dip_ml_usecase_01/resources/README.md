...
The databricks CLI bundles top level is defined by file `dip_ml_usecase_01/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`dip_ml_usecase_01/resources/model-workflow-resource.yml`)
 - batch inference workflow (`dip_ml_usecase_01/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`dip_ml_usecase_01/resources/monitoring-resource.yml`)
 - feature engineering workflow (`dip_ml_usecase_01/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`dip_ml_usecase_01/resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-dip-ml-usecase-01-model     | /dev-dip-ml-usecase-01-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-dip-ml-usecase-01-model | /staging-dip-ml-usecase-01-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-dip-ml-usecase-01-model    | /prod-dip-ml-usecase-01-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-dip-ml-usecase-01-model    | /test-dip-ml-usecase-01-experiment    |
...
2. Complete the `TODO` in `dip_ml_usecase_01/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `dip_ml_usecase_01/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `dip_ml_usecase_01/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-dip-ml-usecase-01-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-dip-ml-usecase-01-batch-inference-job`
that runs the notebook under `dip_ml_usecase_01/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-dip-ml-usecase-01-batch-inference-job`.
...
`test-dip-ml-usecase-01-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-dip-ml-usecase-01-batch-inference-job`.
